...
> 以 **C++（Boost.Asio / gRPC）** 为核心、**Go / Node** 协作的分布式即时通讯系统，配套仿微信前端。
> 功能覆盖实时单聊、**跨服群聊**、消息持久化与历史、**图片 / 文件发送**、离线消息；并经系统化压测——单机双 ChatServer 实测可实时支撑 **1 万条 TCP 长连接**，100 倍 fanout 下实时推送约 **5 万 notify/s**，ACK / Notify 成功率 **100%**。
...
系统实现了一条完整的端到端链路：**邮箱验证码注册 → 登录鉴权 → 服务发现与分配 → 好友搜索 / 申请 / 认证 → 实时文本聊天 → 跨 ChatServer 消息转发 → 离线消息拉取 → 消息持久化与历史 → 图片 / 文件发送 → 跨服群聊 → 心跳保活**。

项目有两条主线值得关注：一是**系统化的性能工程**——我设计并执行了一套自动化压测方案，将系统压到 1 万条长连接，定位出高 fanout 下的推送排队瓶颈，做了业务队列分片、连接池参数化、ACK 前置等优化，形成「压测 → 定位瓶颈 → 优化 → 复测」的闭环；二是**最小侵入的功能演进**——持久化、文件、群聊都坚持「不破坏已有性能、不改 protobuf」，通过复用已有投递与存储原语接入，体现对系统的整体把控。
...
| 构建 | Visual Studio C++ 工程、npm、go build |
...
> 本节是项目相比一般教程作品**最大的差异化亮点**。所有数据均在**本机真实测得**并附原始 JSON，结论刻意区分「实时可用」与「可靠但非实时」，不做夸大。
...
<details>
<summary><b>群聊（跨服 fan-out）</b></summary>

1. 建群：写入 `chat_group` / `chat_group_member`，并向在线成员推「群事件」标记以实时刷新群列表；
2. 发群消息：ACK 发送者 → 持久化到 `chat_message`（`session_id=group_<id>`）→ 取成员列表；
3. 对每个成员按其 `uip_<uid>` 在线集合分发：同服本地推送 / 他服复用 `NotifyTextChatMsg` 转发 / 离线落 Redis；消息体包一层「群标记」让接收端路由到对应群。
</details>

<details>
<summary><b>发送文件 / 图片</b></summary>

1. 浏览器经桥接层上传到 Go FileServer，得到 `file_id` 与元信息；
2. 以一条普通文本消息发送（消息体是文件引用 JSON），复用单聊 / 群聊的投递与持久化；
3. 接收端按引用经桥接层回源：图片内联预览，其它文件渲染为可下载卡片。
</details>
