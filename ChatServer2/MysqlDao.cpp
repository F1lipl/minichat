...
// Batch insert chat messages in a single transaction. INSERT IGNORE together
// with the unique key (session_id, unique_id) makes re-delivered messages
// idempotent. Called only from the async persistence worker, never on the
// request hot path.
bool MysqlDao::SaveChatMsgs(const std::vector<ChatMsgInfo>& msgs) {
	if (msgs.empty()) {
		return true;
	}
	...
	try {
		...
		std::unique_ptr<sql::PreparedStatement> pstmt(con->_con->prepareStatement(
			"INSERT IGNORE INTO chat_message(session_id, from_uid, to_uid, unique_id, content) "
			"VALUES (?, ?, ?, ?, ?)"));

		for (const auto& msg : msgs) {
			pstmt->setString(1, msg.session_id);
			pstmt->setInt(2, msg.from_uid);
			pstmt->setInt(3, msg.to_uid);
			pstmt->setString(4, msg.unique_id);
...
// Cursor-paged chat history for a 1-1 session. last_id == 0 means "latest page".
// Rows are read newest-first (msg_id < cursor) for efficient pagination, then
// reversed so the caller gets ascending chronological order.
bool MysqlDao::GetChatMsgList(const std::string& session_id, long long last_id, int limit,
	std::vector<std::shared_ptr<ChatMsgInfo>>& msg_list) {
	if (limit <= 0 || limit > CHAT_HISTORY_MAX_LIMIT) {
	...
	try {
		std::string sql =
			"SELECT msg_id, session_id, from_uid, to_uid, unique_id, content, status, create_time "
		...
		int idx = 1;
		pstmt->setString(idx++, session_id);
		if (last_id > 0) {
			pstmt->setInt64(idx++, last_id);
		}
		pstmt->setInt(idx++, limit);

		std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());
		while (res->next()) {
			auto info = std::make_shared<ChatMsgInfo>();
			info->msg_id = res->getInt64("msg_id");
			info->session_id = res->getString("session_id");
			info->from_uid = res->getInt("from_uid");
			info->to_uid = res->getInt("to_uid");
			info->unique_id = res->getString("unique_id");
