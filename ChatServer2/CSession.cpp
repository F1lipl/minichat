...
{
	std::lock_guard <std::mutex>lock(_send_lock);
	size_t send_queue_size = _send_que.size();
	if (send_queue_size > MAX_SENDQUE) {
		std::cout << "Session" << _uuid << " send que fulled, size is " << MAX_SENDQUE << std::endl;
		return;
	}
	std::shared_ptr<SendNode>send = std::make_shared<SendNode>(msg.c_str(), msg.length(), msgid);
	_send_que.push(send);
	if (send_queue_size > 0)return;
	auto& msgnode = _send_que.front();
...
void CSession::AsyncReadBody(int total_len)
{
	...
	asyncReadFull(total_len, [self, total_len, this](const boost::system::error_code& ec, size_t bytes_transfered) {
		try {
			if (ec) {
...
void CSession::AsyncReadHead(int totallen) {
	auto self = shared_from_this();
	asyncReadFull(totallen, [self,totallen,this](const boost::system::error_code &ec,size_t bytes_transfered) {
		try {
			if (ec) {
				std::cout << "handle read failed, error is " << ec.what() << endl;
				Close();
...
{
	auto self = shared_from_this();
	_socket.async_read_some(boost::asio::buffer(_data + read_len, total_len - read_len), [self, read_len, total_len,handler](const boost::system::error_code& ec, std::size_t bytesTransfered) {
		if (ec) {
			handler(ec, read_len + bytesTransfered);
			return;
		}
		if (read_len + bytesTransfered>=total_len) {
			handler(ec, total_len);
			return;
		}
...
inline std::shared_ptr<CSession> CSession::SharedSelf()
{
